...
so it's gated by ``PKGSENTRY_FETCH_URL_DEPS`` (default on) for operators who want to
...
MAX_FETCH_BYTES = int(os.environ.get("PKGSENTRY_URL_DEP_MAX_MB", "20")) * 1024 * 1024
FETCH_TIMEOUT = float(os.environ.get("PKGSENTRY_URL_DEP_TIMEOUT", "15"))
MAX_DEPS = int(os.environ.get("PKGSENTRY_URL_DEP_MAX_COUNT", "3"))


def _fetch_enabled() -> bool:
    return os.environ.get("PKGSENTRY_FETCH_URL_DEPS", "1") == "1"
...
async def analyze_url_dependencies(extracted_root: Path) -> list[Finding]:
    ...
    from pkgsentry.ecosystems.npm.installer import _root_package_json_paths
