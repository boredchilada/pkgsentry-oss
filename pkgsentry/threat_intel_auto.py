...
The moat: when a scan is double-confirmed malicious (rules *and* LLM agree), the
fingerprints (SHA-256 + ssdeep + TLSH) of its **implicated files** (the ones that
...
already matches every future file against that table, so the next package reusing
the same (or a *similar* — ssdeep/TLSH) payload matches instantly, even before the
LLM. This turns a one-off catch into campaign-wide recognition (e.g. the meoo-* /
...
_MAX_PER_SCAN = int(os.environ.get("PKGSENTRY_THREATINTEL_MAX_PER_SCAN", "25"))
...
def is_enabled() -> bool:
    # Defaults OFF: a double-confirmed false positive self-seeds and re-confirms on
    # every later release (self-perpetuating FP). Opt in with =1 once a
    # defensive-module guard exists. `threatintel remove` clears an existing seed.
    return os.environ.get("PKGSENTRY_THREATINTEL_AUTOSEED", "0") == "1"
...
def seed_from_scan(
    ...
    try:
        from pkgsentry.watchlist_auto import _blocklist
...
def backfill_tlsh_from_vault(session: Session) -> tuple[int, int]:
    ...
    from pkgsentry.vault import VAULT_PASSWORD, vault_dir
