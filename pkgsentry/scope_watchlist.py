...
# are starting points; operators trim/extend via the `pkgsentry scope` CLI, and
...
# in with PKGSENTRY_SCOPE_WATCH_PROLIFIC=1 when there's headroom.
...
def is_enabled() -> bool:
    return os.environ.get("PKGSENTRY_SCOPE_WATCHLIST", "1") != "0"
...
def seed_baseline(session: Session) -> int:
    ...
    want_prolific = os.environ.get("PKGSENTRY_SCOPE_WATCH_PROLIFIC", "0") == "1"
    inserted = 0
