...
import pkgward.ecosystems  # noqa: F401  -- triggers auto-registration
...
def sync_focus_file(focus_file: str) -> None:
    ...
    from pkgward import focus as focus_mod
    try:
...
async def _async_run(workers: int, duration: int, focus_file: Optional[str] = None) -> None:
    from pkgward.adapter import adapter_registry
    from pkgward.workers import run_pool
    from pkgward.detonation_worker import run_detonation_pool
    ...
    if focus_file:
        os.environ["PKGWARD_FOCUS_EXCLUSIVE"] = "1"
    ...
    from pkgward.util.capabilities import log_capabilities
    log_capabilities()
    ...
    # asyncio.to_thread and loop.getaddrinfo share the loop's default executor,
    # which defaults to min(32, cpus + 4) threads. With every worker's claim +
    # per-scan DB bursts threaded (each ~wire-RTT on a remote-DB node), that
    # pool must hold all workers' DB waits AND DNS lookups concurrently or
    # resolution starves and connects time out.
    det_workers = int(os.environ.get("DETONATION_WORKERS", "6"))
    from concurrent.futures import ThreadPoolExecutor
    loop.set_default_executor(ThreadPoolExecutor(
        max_workers=max(32, workers + det_workers + 16),
    ...
    def _sweep_stale_job():
        from pkgward.queue import sweep_stale_claims
        with sess.session_scope() as s:
    ...
    def _queue_prune_job():
        from pkgward.queue import prune_terminal
        with sess.session_scope() as s:
            n = prune_terminal(s)
            if n:
    ...
    def _maintainer_watch_janitor():
        from pkgward import maintainer_watch
        with sess.session_scope() as s:
            pruned = maintainer_watch.prune(s)
            if pruned:
                log.info("maintainer_watch_janitor", pruned=pruned)

    # Hourly — prunes watches that have hit their release count or TTL backstop.
    scheduler.add_job(_maintainer_watch_janitor, "interval", minutes=60, id="maintainer_watch_janitor")
