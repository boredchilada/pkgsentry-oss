...
class CratesAdapter(EcosystemAdapter):
    ...
    async def discover(self) -> AsyncIterator[DiscoveredItem]:
        from pkgward.ecosystems.crates.ingest.feeds import poll_feeds_once
        # discover() is for parity — real ingest is scheduler-driven
        yield  # pragma: no cover

    async def fetch(self, name: str, version: str) -> FetchResult:
        from pkgward.ecosystems.crates.fetch.download import download_crate
    ...
    def schedule_jobs(self, scheduler) -> None:
        from pkgward.ecosystems.crates.ingest import feeds, watchlist
    ...
    def sweep(self) -> None:
        from pkgward.ecosystems.crates.fetch.download import sweep_orphans
