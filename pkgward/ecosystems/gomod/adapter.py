...
class GoModAdapter(EcosystemAdapter):
    ...
    async def fetch(self, name: str, version: str) -> FetchResult:
        from pkgward.ecosystems.gomod.fetch.download import download_module
    ...
    def schedule_jobs(self, scheduler) -> None:
        from pkgward.ecosystems.gomod.ingest import cursor, watchlist
    ...
    def sweep(self) -> None:
        from pkgward.ecosystems.gomod.fetch.download import sweep_orphans
        sweep_orphans(max_age_seconds=3600.0)

    def backfill(self, days: int) -> int:
        from pkgward.ecosystems.gomod.ingest.cursor import pull_since_beginning
