...
# Benign Go-toolchain introspection in init(): `exec.Command(<go>, "env"/"version", …)`.
# cobra's helpers.go runs `go env GOPATH` this way — developer tooling, not RCE.
_GO_TOOLCHAIN_EXEC = re.compile(r'exec\.Command\w*\([^,]+,\s*"(?:env|version)"')
# Signs the init() exec is NOT just a go-toolchain query: a shell / downloader /
# interpreter literal anywhere in the body (the real init-exec dropper shape).
_NON_GO_EXEC = re.compile(
    r'"(?:/bin/)?(?:sh|bash|dash|zsh|ksh|ash)"'
    r'|"(?:curl|wget|nc|ncat|socat|python[0-9.]*|perl|ruby|powershell|pwsh|cmd|osascript)"'
    r'|"-c"|/bin/sh|exec\.Command\w*\(\s*"[^"]*[;&|]'
)
...
def _analyze_go_file(path: Path, rel_path: str) -> list[Finding]:
    ...
    if init_bodies:
        ...
        if _EXEC_USAGE.search(combined_body):
            # Benign Go-toolchain introspection: cobra's helpers.go (and many CLI
            # frameworks) run `go env GOPATH` from init() to resolve a default
            # GOPATH. That's not arbitrary execution — if the ONLY exec is a
            # `go env`/`go version`-style toolchain query (and init does no network),
            # it's developer tooling, not an install-time RCE. Real init-exec droppers
            # run shells/curl/arbitrary commands, which this still convicts.
            only_go_toolchain = (
                _GO_TOOLCHAIN_EXEC.search(combined_body)
                and not _NON_GO_EXEC.search(combined_body)
                and not _NET_USAGE.search(combined_body)
            )
            if not only_go_toolchain:
                findings.append(Finding(
...
def _analyze_go_mod(path: Path, rel_path: str) -> list[Finding]:
    ...
    for m in _REPLACE_DIRECTIVE.finditer(content):
        target = m.group(2)
        line_num = content[:m.start()].count("\n") + 1
        # Local-path replace. NOTE: Go *ignores* replace directives in any module
        # consumed as a dependency — they apply only in the main module — so a
        # local-path replace in a PUBLISHED module is inert downstream (it cannot
        # substitute code for consumers). It's almost always a monorepo/dev artifact
        # (../sibling sub-modules), not a supply-chain vector. Keep it as a LOW
        # informational signal: recorded as evidence, escalates only when paired with
        # a real behavioral chain (init exec/net), never convicts on its own.
        if target.startswith(".") or target.startswith("/"):
            findings.append(Finding(
                ...
                file=rel_path,
                line=line_num,
                evidence=f"replace directive points to local path (inert for dependents): {m.group(1)} => {target}",
            ))
        else:
            findings.append(Finding(
