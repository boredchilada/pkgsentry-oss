...
class PyPIAdapter(EcosystemAdapter):
    ...
    async def discover(self) -> AsyncIterator[DiscoveredItem]:
        from pkgward.ecosystems.pypi.ingest.feeds import poll_feeds_once
        for item in await poll_feeds_once():
            yield item

    async def fetch(self, name: str, version: str) -> FetchResult:
        from pkgward.ecosystems.pypi.fetch.download import download_all
    ...
    def schedule_jobs(self, scheduler) -> None:
        ...
        from pkgward.ecosystems.pypi.ingest import feeds as feed_mod
    ...
    async def boot(self) -> None:
        ...
        from pkgward.store.models import FocusList, Watchlist
    ...
    def sweep(self) -> None:
        from pkgward.ecosystems.pypi.fetch.download import sweep_orphans
