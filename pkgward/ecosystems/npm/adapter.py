...
class NpmAdapter(EcosystemAdapter):
    ...
    async def fetch(self, name: str, version: str) -> FetchResult:
        from pkgward.ecosystems.npm.fetch.download import download_tarball
    ...
    def schedule_jobs(self, scheduler) -> None:
        from pkgward.ecosystems.npm.ingest import cursor, watchlist
        from pkgward.ecosystems.npm.ingest import focus as nf
    ...
    async def boot(self) -> None:
        ...
        from pkgward.store.models import FocusList, Watchlist as WatchlistModel
    ...
    def sweep(self) -> None:
        from pkgward.ecosystems.npm.fetch.download import sweep_orphans
        sweep_orphans(max_age_seconds=3600.0)

    def backfill(self, days: int) -> int:
        from pkgward.ecosystems.npm.ingest.cursor import pull_since_beginning
