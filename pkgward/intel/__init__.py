...
baseline pack ships in-tree at `pkgward/intel/baseline/`. Operators with
their own tuned threat intel can layer a private overlay via the
`PKGWARD_INTEL_PATH` env var.

Resolution at startup:
  1. Load baseline (always).
  2. If PKGWARD_INTEL_PATH is set + validates, load overlay.
...
def load(overlay_path: Optional[Path] = None, *, use_env: bool = True) -> IntelPack:
    ...
    `overlay_path` overrides the PKGWARD_INTEL_PATH env var when provided
    (mainly for tests). Pass `use_env=False` to force a deterministic
    baseline-only load that ignores PKGWARD_INTEL_PATH (used by the public
    regression corpus, whose golden expectations are pinned to the baseline pack).
    """
    global _current

    if overlay_path is None and use_env:
        env_path = os.environ.get("PKGWARD_INTEL_PATH", "").strip()
        if env_path:
