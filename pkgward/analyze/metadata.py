...
from pkgward.analyze.lure_names import analyze_lure_name
...
def file_list_mismatch(sdist_files: list[str], wheel_files: list[str]) -> list[Finding]:
    ...
    # Only meaningful for a real PyPI dual-artifact release (both an sdist AND a
    # wheel). gomod/crates/npm ship a single archive, and a PyPI release with only
    # one artifact has nothing to compare against — an empty counterpart would make
    # *every* file "absent from sdist" and fire on every such package (the
    # gomod/Spitzer FP). Require both sides present before diffing.
