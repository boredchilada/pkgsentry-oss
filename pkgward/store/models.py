...
class Package(Base):
    ...
    # refreshed at alert time with a TTL. See pkgward/enrich/downloads.py.
...
class FocusList(Base):
    ...
    ``PKGWARD_FOCUS_EXCLUSIVE=1`` the scanner ingests ONLY these packages.
    """
...
class MaintainerWatch(Base):
    """Bounded force-scan watch on a caught maintainer's *clean* sibling packages.

    When a package is double-confirmed malicious, the maintainer pivot force-scans
    that account's whole catalog. A one-shot sweep only catches siblings already
    poisoned at sweep time; an account that rolls its payload out over several
    releases would slip back into the "established-package update → skipped at
    ingest" blind spot. This table closes that temporal hole: each clean sibling is
    watched so its next few releases are force-scanned at high priority.

    FORCE-SCAN ONLY — this is NOT a known-bad mark (no sentinel rank, no
    ``known_bad_deps``); it only gets a package *looked at*. Bounded two ways: a
    release count (the janitor prunes once ``PKGWARD_MAINTAINER_WATCH_RELEASES``
    distinct versions have been scanned since ``added_at``) and a safety TTL
    (``PKGWARD_MAINTAINER_WATCH_TTL_DAYS``) so a never-releasing entry still falls
    off. The gate check is pure set membership; the count/TTL bounds live in the
    janitor (``maintainer_watch.prune``).
    """

    __tablename__ = "maintainer_watch"
    id: Mapped[int] = mapped_column(primary_key=True)
    ecosystem: Mapped[str] = mapped_column(String(32), nullable=False, default="pypi")
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    maintainer: Mapped[Optional[str]] = mapped_column(String(128), nullable=True)
    added_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utcnow)

    __table_args__ = (
        UniqueConstraint("ecosystem", "name", name="uq_maintainerwatch_ecosystem_name"),
        Index("ix_maintainerwatch_ecosystem", "ecosystem"),
    )
