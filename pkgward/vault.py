...
The vault is a private operator asset: enabled only when ``$PKGWARD_VAULT_PATH``
...
def vault_dir() -> Optional[Path]:
    p = os.environ.get("PKGWARD_VAULT_PATH", "").strip()
...
def list_vaulted() -> list[dict]:
    """Enumerate vaulted samples by reading their manifests.

    Returns dicts with ecosystem/name/version/archive_kind/verdict/score — used by the
    ``replay`` CLI to re-run known-bad samples through the real pipeline."""
    vdir = vault_dir()
    if vdir is None:
        return []
    import glob
    ...
    out: list[dict] = []
    for mf in sorted(glob.glob(str(vdir / "*.manifest.toml"))):
        try:
            with open(mf, "rb") as fh:
                m = tomllib.load(fh)
            out.append({
                "ecosystem": m["ecosystem"], "name": m["name"], "version": m["version"],
                "archive_kind": m.get("archive_kind", ""),
                "verdict": m.get("expected_verdict", ""), "score": m.get("score", 0),
            })
        except Exception as e:  # noqa: BLE001
            log.warning("vault_manifest_unreadable", manifest=mf, error=str(e))
    return out
