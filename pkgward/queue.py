...
QUEUE_RETENTION_DAYS = int(os.environ.get("PKGWARD_QUEUE_RETENTION_DAYS", "14"))
...
ENQUEUE_DEADLOCK_RETRIES = int(os.environ.get("PKGWARD_ENQUEUE_DEADLOCK_RETRIES", "3"))
...
def claim_next(session: Session) -> Optional[tuple[ScanQueue, str]]:
    ...
    for prio in ("high", "normal", "low"):
        ...
        for eco in _weighted_order(ecosystems, weights):
            ...
            # Lost ids are excluded explicitly: under snapshot isolation (SQLite,
            # REPEATABLE READ) the re-select would keep returning the same
            # already-claimed row, burning every retry and falling through a
            # priority tier while claimable rows remain.
            ...
            for _ in range(_CLAIM_CAS_RETRIES):
                ...
                if lost_ids:
                    stmt = stmt.where(ScanQueue.id.not_in(lost_ids))
